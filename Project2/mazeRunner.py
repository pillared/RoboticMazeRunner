...
#TODOCODE################################################################333
def outertrace(trigger):
    gpg = EasyGoPiGo3()
    dist = gpg.init_distance_sensor()
    servoHor = gpg.init_servo('SERVO1')
    servoVer = gpg.init_servo('SERVO2')               
    servoHor.rotate_servo(90)
    servoVer.rotate_servo(180)
    ...
    def loopy(FLAG,i, d=999):
        gpg.reset_encoders()
        
        servoHor.rotate_servo(180)
        servoVer.rotate_servo(178)
        
        #drive autonomously until Ctrl-C pressed
        #check if an obstacle faced within 250mm (25cm)
        while not (trigger.is_set() or FLAG > 6):
            gpg.forward()
            d = dist.read_mm()
            if(d >= 3000):
                if not (FLAG % 3 == 0):
                    gpg.stop()
                    #distancemoved = round(gpg.read_encoders_average())
                    print(encoders_read)
                    gpg.drive_cm(15)
                    #distancemoved+=15
                    print(FLAG)
...
def checkAngle():
    ...
    if(r1 >= r2):
        r = r1-r2
        ang = math.degrees(math.atan((r)/(20)))
        gpg.turn_degrees(-90)
        gpg.drive_cm((r)/(20))
        gpg.turn_degrees(90-ang)
    elif(r2 >= r1):
        l = l1-l2
        ang = math.degrees(math.atan((l)/(20)))
        gpg.turn_degrees(90)
        gpg.drive_cm((l)/(20))
        gpg.turn_degrees(-(90-ang))
...
def start(trigger):
    ...
    while move is True:
        ...
        if(dist.read_mm() <= 120):
            gpg.stop()
            distancemoved = round(gpg.read_encoders_average()) + 12
            
            servoHor.rotate_servo(180) 
            sleep(.35)
            checkLeft = dist.read_mm()
            
            servoHor.rotate_servo(0) 
            sleep(.35)
            checkRight = dist.read_mm()
        
            print(checkLeft)
            print(checkRight)
            if(((checkLeft + checkRight)/400) <= 1.5):
                gpg.turn_degrees(-180)
                gpg.drive_cm(30)
                print("Hit it from the back!")
                ...
                if(checkLeft >= checkRight and checkLeft > 80):
                    gpg.turn_degrees(-90)
                    gpg.forward()
                    print("Left!")
                elif(checkRight >= checkLeft and checkRight > 80):
                    gpg.turn_degrees(90)
                    gpg.forward()
                    print("Right!")
            elif(checkLeft >= checkRight and checkLeft > 150):
                gpg.turn_degrees(-90)
                gpg.drive_cm(30)
                #checkAngle()
                gpg.forward()
                print("Left!")
            elif(checkRight >= checkLeft and checkRight > 150):
                gpg.turn_degrees(90)
                gpg.drive_cm(30)
                #checkAngle()
                gpg.forward()
