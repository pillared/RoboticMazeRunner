...
def Main():
    ...
    while robot_operating:
        ...
        # if the robot is closer to the target
            ...
            color="Red"
            gopigo3.stop()
            gpg.set_eye_color((225,0,0))
            gpg.open_eyes()
            time.sleep(1)
            gpg.close_eyes()
            #gpg.set_eye_color((255,255,102))
            #gpg.open_eyes()
            #gopigo3.backward()
            #time.sleep(1)
            #gpg.close_eyes
        ...
        # if the robot is far away from the target
            ...
            gopigo3.forward()
            color="Green"
            gpg.set_eye_color((0,225,0))
            gpg.open_eyes()

        # and last, print some stats
        print("Current distance : {:4} mm Current speed: {:4} Stopped: {:4} Light: {:4}".format(current_distance, int(determined_speed), gopigo3_stationary is True ,color))
