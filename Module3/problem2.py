...
def Main():
    ...
    while robot_operating:
        ...
        # if the robot is far away from the target
            ...
            #for i in range(2):
            print(" Index{:4} Current distance : {:4} mm Encoder Val: {:4} Stopped: {}".format(i, current_distance, int(determined_speed), gopigo3_stationary is True ))
            file =open("./problem2_pathtrace.csv","a+")
            file.write(("Index{:4}Current distance : {:4} mm Encoder Val: {:4} Stopped: {}  \r\n".format(i, current_distance, int(determined_speed), gopigo3_stationary is True )))
            i+=1
